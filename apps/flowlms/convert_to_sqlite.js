...
// Write the SQLite config and connection
const dbIndex = `import { drizzle } from 'drizzle-orm/libsql';
...
`;
fs.writeFileSync(path.join(__dirname, 'src/lib/db/index.ts'), dbIndex);
...
`;
fs.writeFileSync(path.join(__dirname, 'drizzle.config.ts'), drizzleConfig);

const envFile = \`DATABASE_URL=file:local.db
BETTER_AUTH_SECRET=super-secret-key-12345
BETTER_AUTH_URL=http://localhost:3000
\`;
fs.writeFileSync(path.join(__dirname, '.env'), envFile);

console.log('Successfully updated schema to SQLite.');
